...
namespace Lib.Core
{
    ...
    /// 缓存管理类
    /// </summary>
    public static partial class MngCache
    {
        ...
        static MngCache()
        {
            try
            {
                string[] fileNameList = Directory.GetFiles(System.Web.HttpRuntime.BinDirectory, "hnliving.CacheStrategy.*.dll", SearchOption.TopDirectoryOnly);
                _icachestrategy = (ICacheStrategy)Activator.CreateInstance(Type.GetType(string.Format("hnliving.CacheStrategy.{0}.CacheStrategy, hnliving.CacheStrategy.{0}", fileNameList[0].Substring(fileNameList[0].IndexOf("CacheStrategy.") + 14).Replace(".dll", "")),
                                                                                       false,
                                                                                       true));
            }
            catch
            {
                throw new BaseException("创建'缓存策略对象'失败，可能存在的原因：未将'缓存策略程序集'添加到bin目录中；'缓存策略程序集'文件名不符合'hnliving.CacheStrategy.{策略名称}.dll'格式");
            }
